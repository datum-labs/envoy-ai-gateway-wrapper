  ...
  return (
    <Card className="lift flex h-full flex-col gap-4 p-4">
      <div className="flex flex-col gap-0.5">
        <h2 className="font-title text-base font-medium text-foreground">
          {title}
        </h2>
        {description && (
...
export function OverviewView({ initial }: { initial?: OverviewPayload }) {
  ...
  const { data } = useSWR<OverviewPayload>(
  ...
  const t = o?.totals;
  const metrics = data?.source?.metrics;
  const description =
    metrics === "live"
      ? ""
      : metrics === "unavailable"
        ? "Gateway connected, but metrics are unavailable. Set ENVOY_AI_GATEWAY_METRICS_URL to a Prometheus base or /metrics scrape endpoint."
        : "Traffic, spend, and health across your Envoy AI Gateway.";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Reveal index={3}>
          <StatCard
            ...
            deltaLabel={
              o
                ? formatSignedPercent(o.deltas.errorRate * 100) + " pts"
                : undefined
            }
            invertDelta
          />
      ...
      <Reveal fade delay={50} className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <ChartCard
            ...
            description="Successful requests vs errors"
          >
      ...
      <Reveal fade delay={90} className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <ChartCard
          ...
          description="USD when the gateway reports pricing"
        >
